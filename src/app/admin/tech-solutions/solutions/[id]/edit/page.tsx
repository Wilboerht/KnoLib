...
export default function EditTechSolution({ params }: PageProps) {
  ...
  useEffect(() => {
    ...
    const loadSolution = async () => {
      try {
        setLoading(true);
        const response = await fetch(`/api/tech-solutions/${solutionId}`);
        const data = await response.json();
        ...
        if (data.success) {
          ...
          setErrors({ general: "Solution not found" });
        }
      } catch (error) {
        console.error('Error loading solution:', error);
        setErrors({ general: "Failed to load solution" });
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrors({ general: 'Failed to update solution. Please try again.' });
    } finally {
      setSaving(false);
    }
  };
    ...
    return (
      <div className="min-h-screen bg-gray-50 dark:bg-slate-900 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600 dark:text-gray-300">Loading solution...</p>
        </div>
      </div>
    );
  }
  ...
  if (!solution) {
    return (
      <div className="min-h-screen bg-gray-50 dark:bg-slate-900 flex items-center justify-center">
        <div className="text-center">
          <p className="text-red-600 dark:text-red-400 mb-4">Solution not found</p>
        ...
        <div className="py-8">
          ...
          >
            ...
            <p className="text-gray-600 dark:text-gray-300 mt-2">
              Update solution information and content
            </p>
          </motion.div>
